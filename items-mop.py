...
def parse_item_data(html): 
    ...
    for table in soup.find_all('table'):
        ...
        if tds and span:
            ...
            # Only accept if the slot is valid (case-insensitive)
            # If the span text is an armor type, reverse the order
            if td_text.lower() in valid_slots and span_text.lower() in armor_types:
                print(f"{span_text} {td_text}")
                
            elif td_text.lower() in valid_slots:
                print(f"{td_text} {span_text}")
